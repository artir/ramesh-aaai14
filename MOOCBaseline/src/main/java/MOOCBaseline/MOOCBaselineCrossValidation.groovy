...
m.add rule : ( viewActivity(U) ) >> performance(U),  weight : 10
m.add rule : ( user(U) & ~viewActivity(U) ) >> ~performance(U),  weight : 10

//all activity indicates performance
//m.add rule : ( postActivity(U) & viewActivity(U) & voteActivity(U) ) >> performance(U),  weight : 5
//m.add rule : ( postActivity(U) & viewActivity(U) ) >> performance(U),  weight : 5
...
//m.add rule : ( posts(U, P)) >> performance(U),  weight : 5
//
//m.add rule : ( posts(U, P) & subjective(P)) >> performance(U),  weight : 5
//m.add rule : ( posts(U, P) & post(P) & ~subjective(P)) >> ~performance(U),  weight : 5
//m.add rule : ( posts(U, P) & polarity(P) & subjective(P) ) >> performance(U),  weight : 5
//
...
m.add rule : ( posts(U1, P1) & posts(U2, P2) & user(U1) & ~performance(U1) & inThread(P1, T) & inThread(P2, T) ) >> ~performance(U2),  weight : 5
...
def trainDir = 'data'+java.io.File.separator+'women'+java.io.File.separator + 'start-end-all' + java.io.File.separator//+'till-t'+(timePeriod-1)+java.io.File.separator
...
for (Predicate p : [user, post, upvote, downvote, posts, inthread, lastQuiz, lastPost, lastLecture, lastView])
{
	println "\t\t\tREADING " + p.getName() +" ...";
	insert = data.getInserter(p, trainPart)
...
List<List<Double []>> resultsDiscrete = new ArrayList<List<Double []>>()
resultsDiscrete.add(new ArrayList<Double []>())
...
//		if (i != trainingTarget && i != fold)
	...
	Database trainDB = data.getDatabase(trainWritePartitions.get(fold),[postActivity, ontime, reputation, polarity, subjective, submittedlecture, user, post, upvote, downvote, posts, viewActivity, voteActivity, lastQuiz, lastPost, lastLecture, lastView] as Set, (Partition []) trainPart)
	Database testDB = data.getDatabase(testWritePartitions.get(fold), [postActivity, ontime, reputation, polarity, subjective, submittedlecture, user, post, upvote, downvote, posts, viewActivity, voteActivity, lastQuiz, lastPost, lastLecture, lastView] as Set, (Partition []) testReadPartitions.toArray())
...
//	}
	...
	def discreteComparator = new DiscretePredictionComparator(resultsDB);
	discreteComparator.setBaseline(groundTruthDB)
...
//	}
	...
	//def discreteMetrics = [DiscretePredictionStatistics.getPrecision(BinaryClass.NEGATIVE), DiscretePredictionStatistics.getF1(BinaryClass.NEGATIVE)]
	
	double [] score = new double[metrics.size()]
	double [] discreteScore = new double[6]
	for (int i = 0; i < metrics.size(); i++) {
		comparator.setRankingScore(metrics.get(i))
		score[i] = comparator.compare(performance)
	}
	
	def discreteMetric = discreteComparator.compare(performance)
	
	discreteScore[0] = discreteMetric.getPrecision(BinaryClass.POSITIVE);
	discreteScore[1] = discreteMetric.getRecall(BinaryClass.POSITIVE);
	discreteScore[2] = discreteMetric.getPrecision(BinaryClass.NEGATIVE);
	discreteScore[3] = discreteMetric.getRecall(BinaryClass.NEGATIVE);
	discreteScore[4] = discreteMetric.getF1(BinaryClass.POSITIVE);
	discreteScore[5] = discreteMetric.getF1(BinaryClass.NEGATIVE);
...
//	}
	...
	resultsDiscrete.get(0).add(fold, discreteScore)
	resultsDiscrete.get(0)
...
def methodDiscreteStats = resultsDiscrete.get(0)
sum = new double[4];
sumSq = new double[4];
sumDiscrete = new double[6];
sumSqDiscrete = new double[6];
for (int fold = 0; fold < folds; fold++) {
	def score = methodStats.get(fold)
	def discreteScore = methodDiscreteStats.get(fold)
	for (int i = 0; i < 4; i++) {
		sum[i] += score[i];
		sumSq[i] += score[i] * score[i];
	}
	for (int i=0; i<6; i++) {
		sumDiscrete[i] += discreteScore[i];
		sumSqDiscrete[i] += discreteScore[i] * discreteScore[i];
	}
	System.out.println("Method " + config + ", fold " + fold +", auprc positive: "
			+ score[0] + ", negative: " + score[1] + ", auROC: " + score[2] + ", kendall: " + score[3])
//	System.out.println("Method " + config + ", fold " + fold +", Pos Prec: "
//		+ discreteScore[0] + ", Pos Rec: " + discreteScore[1] + ", Neg Prec: " + discreteScore[2] + ", Neg Rec: " + discreteScore[3] + ", Pos F1: "+ discreteScore[4]+ 
//		+ "Neg F1 : "+ discreteScore[5]);
}
...
meanDiscrete = new double[6];
varianceDiscrete = new double[6];
for (int i=0; i<6; i++) {
	meanDiscrete[i] = sumDiscrete[i] / folds;
	varianceDiscrete[i] = sumSqDiscrete[i] / folds - meanDiscrete[i] * meanDiscrete[i];
}
System.out.println();
System.out.println("auprc positive: (mean/variance) "
		+ mean[0] + "  /  " + variance[0] );
System.out.println("auprc negative: (mean/variance) "
		+ mean[1] + "  /  " + variance[1] );
System.out.println("auROC: (mean/variance) "
		+ mean[2] + "  /  " + variance[2] );
System.out.println("kendall: (mean/variance) "
		+ mean[3] + "  /  " + variance[3 ] );
	
System.out.println("Method " + config + ", pos precision "
		+ meanDiscrete[0] + "  /  " + varianceDiscrete[0] );
System.out.println("Method " + config + ", pos recall "
		+ meanDiscrete[1] + "  /  " + varianceDiscrete[1] );
System.out.println("Method " + config + ", neg precision "
		+ meanDiscrete[2] + "  /  " + varianceDiscrete[2] );
System.out.println("Method " + config + ", neg recall "
		+ meanDiscrete[3] + "  /  " + varianceDiscrete[3 ] );
System.out.println("Method " + config + ", f1 pos "
		+ meanDiscrete[4] + "  /  " + varianceDiscrete[2] );
System.out.println("Method " + config + ", f1 neg "
		+ meanDiscrete[5] + "  /  " + varianceDiscrete[3 ] );
System.out.println();
