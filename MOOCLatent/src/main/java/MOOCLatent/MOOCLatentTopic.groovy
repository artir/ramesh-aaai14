...
m.add rule : ( posts(U, P) & POSTCOURSETOPIC(P)) >> engagement_active(U),  weight : 5
m.add rule : ( posts(U, P) & POSTGENERALTOPIC(P) & POST(P) & ~POLARITY(P) ) >> ~engagement_active(U),  weight : 5
m.add rule : ( posts(U, P) & POSTLOGISTICS(P) & POST(P) & ~POLARITY(P) ) >> ~engagement_active(U),  weight : 5
...
m.add rule : ( lastQuiz(U, "3") ) >> disengagement(U),  weight : 10

m.add rule : ( lastPost(U, "2") ) >> disengagement(U),  weight : 10
m.add rule : ( lastPost(U, "3") ) >> disengagement(U),  weight : 10

//m.add rule : ( lastPost(U, "2") & lastQuiz(U, "2") ) >> disengagement(U),  weight : 10
//m.add rule : ( lastPost(U, "3") & lastQuiz(U, "3") ) >> disengagement(U),  weight : 10
...
m.add rule : ( coursetopic(U) ) >> engagement_active(U),  weight : 10
m.add rule : ( logistics(U) ) >> engagement_passive(U),  weight : 10
m.add rule : ( generaltopic(U) ) >> ~engagement_active(U),  weight : 5
m.add rule : ( ~coursetopic(U) ) >> ~engagement_active(U),  weight : 5
...
def trainDir = 'data'+java.io.File.separator+'full'+java.io.File.separator//+'t2'+java.io.File.separator
...
for (Predicate p : [user, post, upvote, downvote, posts, forumType, inForum, lastLecture, lastPost, lastQuiz, lastVote, lastView])
{
	println "\t\t\tREADING " + p.getName() +" ...";
	insert = data.getInserter(p, trainPart)
...
//		if (i != trainingTarget && i != fold)
	...
	Database trainDB = data.getDatabase(trainWritePartitions.get(fold),[postActivity, ontime, reputation, polarity, subjective, submitted, user, post, upvote, downvote, posts, viewActivity, voteActivity, forumType, inThread, inForum, lastQuiz, lastPost, lastLecture, lastView, lastVote, COURSETOPIC, GENERALTOPIC, LOGISTICS, POSTCOURSETOPIC, POSTGENERALTOPIC, POSTLOGISTICS] as Set, (Partition []) trainPart)
	Database testDB = data.getDatabase(testWritePartitions.get(fold), [postActivity, ontime, reputation, polarity, subjective, submitted, user, post, upvote, downvote, posts, viewActivity, voteActivity, forumType, inThread, inForum, lastQuiz, lastPost, lastLecture, lastView, lastVote, COURSETOPIC, GENERALTOPIC, LOGISTICS, POSTCOURSETOPIC, POSTGENERALTOPIC, POSTLOGISTICS] as Set, (Partition []) testReadPartitions.toArray())
...
//	}
	...
	DataOutputter.outputPredicate("output/results-engagement-active" + fold + ".val" , testDB, engagement_active, ",", true, "User,Truth");
	testDB.close()
