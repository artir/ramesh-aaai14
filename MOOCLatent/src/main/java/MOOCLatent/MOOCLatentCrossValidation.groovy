...
ConfigBundle config = cm.getBundle("mooc-latent-1")
Logger log = LoggerFactory.getLogger(this.class)
ExperimentConfigGenerator configGenerator = new ExperimentConfigGenerator("mooc-latent-1");
configGenerator.setLearningMethods(["MLE", "MPLE", "MM"]);

//database

def defaultPath = System.getProperty("java.io.tmpdir")
String dbpath = config.getString("dbpath", defaultPath + File.separator + "mooc-latent-1")
...
m.add rule : ( user(U) & ~engagement_active(U) & engagement_passive(U)) >> ~performance(U),  weight : 2
m.add rule : ( user(U) & ~engagement_active(U) ) >> ~performance(U), weight : 5
...
def trainDir = 'data'+java.io.File.separator+'disruptive-tech'+java.io.File.separator
Partition trainPart = new Partition(0)
Partition targetPart = new Partition(1)
for (Predicate p : [postActivity, ontime, reputation, submitted, viewActivity, voteActivity])
{
...
for (Predicate p : [user, post, upvote, downvote, posts, inForum])
{
	println "\t\t\tREADING " + p.getName() +" ...";
	insert = data.getInserter(p, trainPart)
...
//		if (i != trainingTarget && i != fold)
	...
	Database trainDB = data.getDatabase(trainWritePartitions.get(fold),[postActivity, ontime, reputation, polarity, subjective, submitted, user, post, upvote, downvote, posts, viewActivity, voteActivity, forumType, inThread, inForum] as Set, (Partition []) trainPart)
	Database testDB = data.getDatabase(testWritePartitions.get(fold), [postActivity, ontime, reputation, polarity, subjective, submitted, user, post, upvote, downvote, posts, viewActivity, voteActivity, forumType, inThread, inForum] as Set, (Partition []) testReadPartitions.toArray())
