...
ConfigBundle config = cm.getBundle("mooc-temporal-1")
Logger log = LoggerFactory.getLogger(this.class)
ExperimentConfigGenerator configGenerator = new ExperimentConfigGenerator("mooc-temporal-1");
configGenerator.setLearningMethods(["MLE", "MPLE", "MM"]);

//database

def defaultPath = System.getProperty("java.io.tmpdir")
String dbpath = config.getString("dbpath", defaultPath + File.separator + "mooc-temporal-1")
...
m.add rule : ( postActivity(U) & viewActivity(U) ) >> performance(U),  weight : 7
...
m.add rule : ( posts(U, P) & polarity(P)) >> engagement_active(U),  weight : 7
m.add rule : ( posts(U, P) & subjective(P)) >> engagement_active(U),  weight : 7
m.add rule : ( posts(U, P) & post(P) & ~polarity(P)) >> ~engagement_active(U),  weight : 5

//m.add rule : ( posts(U, P) & inThread(P, T) & inForum(T, F) & forumType(F, "COURSE") ) >> engagement_active(U),  weight : 10
//m.add rule : ( posts(U, P) & inThread(P, T) & inForum(T, F) & forumType(F, "LOGISTICS") & POST(P) & ~POLARITY(P) ) >> ~engagement_active(U),  weight : 5
//m.add rule : ( posts(U, P) & inThread(P, T) & inForum(T, F) & forumType(F, "GENERAL") & POST(P) & ~POLARITY(P) ) >> ~engagement_active(U),  weight : 5
...
//temporal rules
m.add rule : ( lastLecture(U, "1") ) >> disengagement(U),  weight : 10
m.add rule : ( lastQuiz(U, "1") ) >> disengagement(U),  weight : 10
m.add rule : ( lastPost(U, "1") ) >> disengagement(U),  weight : 10
//
m.add rule : ( lastLecture(U, "2") ) >> disengagement(U),  weight : 10
m.add rule : ( lastQuiz(U, "2") ) >> disengagement(U),  weight : 10
m.add rule : ( lastPost(U, "2") ) >> disengagement(U),  weight : 10
////
//////
m.add rule : ( lastPost(U, "2") ) >> engagement_active(U),  weight : 3
m.add rule : ( lastQuiz(U, "2") ) >> engagement_active(U),  weight : 3
m.add rule : ( lastLecture(U, "2") ) >> engagement_passive(U),  weight : 3

m.add rule : ( lastPost(U, "3") ) >> engagement_active(U),  weight : 10
m.add rule : ( lastQuiz(U, "3") ) >> engagement_active(U),  weight : 10
m.add rule : ( lastLecture(U, "3") ) >> engagement_passive(U),  weight : 10

m.add rule : ( lastPost(U, "3") ) >> disengagement(U),  weight : 3
m.add rule : ( lastQuiz(U, "3") ) >> disengagement(U),  weight : 3
m.add rule : ( lastLecture(U, "3") ) >> disengagement(U),  weight : 3

m.add rule : ( lastPost(U, "2") & lastQuiz(U, "2") ) >> disengagement(U),  weight : 10
m.add rule : ( lastPost(U, "1") & lastQuiz(U, "1") ) >> disengagement(U),  weight : 10
...
m.add rule : ( user(U) & ~engagement_active(U) & engagement_passive(U)) >> performance(U),  weight : 5
m.add rule : ( user(U) & engagement_active(U) & ~engagement_passive(U)) >> performance(U),  weight : 5
m.add rule : ( user(U) & ~engagement_passive(U) ) >> ~performance(U), weight : 10
m.add rule : ( user(U) & ~engagement_active(U) ) >> ~performance(U), weight : 7
m.add rule : ( user(U) & disengagement(U) ) >> ~performance(U), weight : 10

m.add rule : ( user(U) & ~submittedlecture(U) ) >> ~engagement_active(U),  weight : 10
//m.add rule : ( user(U) & ~ontime(U) ) >> ~engagement_active(U),  weight : 10

m.add rule : ( submittedlecture(U) ) >> engagement_passive(U),  weight : 5
m.add rule : ( submittedlecture(U) ) >> engagement_active(U),  weight : 5
...
m.add rule : ( engagement_active(U) & submittedLecture(U) ) >> performance(U),  weight : 5
...
def course = 'gene'
def timePeriod = 'end'
def trainDir = 'data'+java.io.File.separator+course+java.io.File.separator+timePeriod+'-all-eng'+java.io.File.separator//+'t2'+java.io.File.separator
Partition trainPart = new Partition(0)
Partition targetPart = new Partition(1)
for (Predicate p : [postActivity, reputation, submittedlecture, viewActivity, voteActivity])
{
...
//		if (i != trainingTarget && i != fold)
	...
	Database trainDB = data.getDatabase(trainWritePartitions.get(fold),[postActivity, reputation, polarity, subjective, submittedlecture, user, post, upvote, downvote, posts, viewActivity, voteActivity, forumType, inThread, inForum, lastQuiz, lastPost, lastLecture, lastView, subscribethreads, subscribeforums] as Set, (Partition []) trainPart)
	Database testDB = data.getDatabase(testWritePartitions.get(fold), [postActivity, reputation, polarity, subjective, submittedlecture, user, post, upvote, downvote, posts, viewActivity, voteActivity, forumType, inThread, inForum, lastQuiz, lastPost, lastLecture, lastView,  subscribethreads, subscribeforums] as Set, (Partition []) testReadPartitions.toArray())
	...
	DataOutputter.outputPredicate("output/training-truth" +course+"-"+timePeriod+ fold + ".val" , labelsDB, performance, ",", true, "User,Truth");
...
//	println "Engagement active"
//	for (GroundAtom atom : Queries.getAllAtoms(testDB, engagement_active)){
//		if (testUsers.contains(atom.getArguments()[0].toString()))
//			println atom.toString() + "\t" + atom.getValue();
//	}
	DataOutputter.outputPredicate("output-all/"+course+"/"+timePeriod+"/"+"results-engagement-active-" +course+"-"+timePeriod+ fold + ".val" , testDB, engagement_active, ",", true, "User,Truth");
	DataOutputter.outputPredicate("output-all/"+course+"/"+timePeriod+"/"+"results-engagement-passive-" +course+"-"+timePeriod+ fold + ".val" , testDB, engagement_passive, ",", true, "User,Truth");
	DataOutputter.outputPredicate("output-all/"+course+"/"+timePeriod+"/"+"results-disengagement-" +course+"-"+timePeriod+ fold + ".val" , testDB, disengagement, ",", true, "User"+"\t"+"Truth");
	DataOutputter.outputPredicate("output-all/"+course+"/"+timePeriod+"/"+"results-performance-" +course+"-"+timePeriod+ fold + ".val" , testDB, performance, ",", true, "User"+"\t"+"Truth");
	testDB.close()
